...
Rajouter des fonctionnalités à l’application que nous avons reçue, par exemple;
...
Ainsi que d'autres fonctionnalité que nous mettrons si le temps nous le permet.
...
Notre application a été écrite en java avec la bibliothèque de java Fx 1.8 qui nous a permis de créer l'interface graphique de l'application, ainsi qu'une base de données en SQL oracle qui nous permet de stocker tous les clients de la banque, leurs comptes, leurs transactions, les employés. Nous avons choisi SQL oracle car il est simple d'utilisation.


image::Image/LogoJAVAFX.png[width=10%]
image::Image/LogoSql.png[width=10%]

Nous avons créé un .jar permettant d'exécuter l'application sur un Pc.

Plusieurs packages sont disponibles dans notre application, chacun joue un rôle important, une certaine hiérarchie est instaurée.

image::Image/packages.png[width=10%]

Application.view :: regroupe les fichiers fxml ainsi que leurs différents controllers.
Application.tools :: est un package contenant les outils d'alertes, des constantes réutilisables.
Application.control :: permet de gérer les différentes fenêtres créer dans le package view.
Application :: permet de lancer les différentes méthodes pour initialiser le lancement de l'application

Model.data :: regroupe tous les différents acteurs qui peuvent interagir et qui sont stockés dans la base de données, ainsi que leur different constructeur
Par exemple les clients, les employés.

Model.orm :: Permets de gérer les différentes actions qu'on peut réaliser depuis l'application comme le CRUD.

Model.orm.exception :: lève toutes les exceptions rencontrées dans l'application

== 2.Présentation des différentes versions 
...
*** **modifierEmploye** (Méthode modifie un employé existant) => **EmployeManagementController** (Classe qui l'appelle). 
*** **nouveauEmployé** (Méthode ajoute un nouvel employé) => **EmployeEditorPaneController** (Classe qui l'appelle).
*** **getListeEmployé** (Méthode recupere la liste de tous les employés) => **EmployeManagementController** (Classe qui l'appelle).

** Classe : **EmployeEditorPane** Permet l'ouverture de la fenêtre pour modifier un employé elle est appelé dans la classe: 
*** **EmployeEditorPane** (Méthode pour parametre la fenetre) => **EmployesManagement** (Classe qui l'appele) => **EmployeManagementController** (Classe qui l'appelle) 
...
*** **getEmploye** (Méthode pour récuperer 1 employé) => **DailyBankMainFrame** (Classe qui l'appelle).
*** **insertEmploye** (Méthode pour ajouter des employés) => **EmployeManagement** (Classe qui l'appelle)  => **EmployeEditorPaneController** (Classe qui l'appelle).
*** **updateEmploye** (Méthode pour modifier des employés) => **EmployeManagement** (Classe qui l'appelle)  => **EmployeEditorPaneController** (Classe qui l'appelle).
*** **getEmploye1** (Méthode pour récuperer tous les employés)  => **EmployeManagement** (Classe qui l'appelle)
...
C'est sur cette interface que l'opération virement et disponible.
